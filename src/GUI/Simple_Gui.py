...
def layout():
    ...
    layout = [[sg.Input(default_text='Video URL or Local Path:', size=(30, 1), key='-VIDEO_LOCATION-'), sg.Button('load')],
              ...
              [btn('previous'), btn('play'), btn('next'), btn('pause'), btn('stop'),btn('exit') ],
              [sg.Text('Load media to start', key = '-MESSAGE_AREA-')]              
              ]
    
    window = sg.Window('SmartMedia Player', layout, element_justification='center', finalize=True, resizable=True)
    ...
    return window, list_player, media_list, player



# def event_loop(window, list_player, media_list, player):
